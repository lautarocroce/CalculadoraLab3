...
namespace CalculadoraFront.Servicios
{
    namespace CalculadoraEjercicio.Servicios
    {
        ...
        internal class Operador
        {
                    ...
                    if (ecuacion.Contains('/'))
                    {
                        ...
                        for (int i = ecuacion.IndexOf('/') + 1; i < ecuacion.Length; i++)
                        {
                            ...
                            */
                            ...
                            if (numero2.Equals("") && ecuacion[i].Equals('-'))
                            {
                                numero2 += ecuacion[i];
                            }
                            else if (numeros.Contains(ecuacion[i]) && !(ecuacion[i].Equals('-')))
                            {
                                numero2 += ecuacion[i];
                            }
                            else if (signos.Contains(ecuacion[i]) || ecuacion[i].Equals('-'))
                            {
                                break;
                            }
            ...
            {
                //int contador = 0;
                while (ecuacion.Contains('-') && ecuacion.LastIndexOf('-') != 0 && numeros.IndexOf(ecuacion[ecuacion.LastIndexOf('-') - 1]) != -1 )
                {
                    String numero1 = "";
                    String numero2 = null;
                    ...
                    for (int i = ecuacion.LastIndexOf('-') - 1; i >= 0; i--)
                    {
                        ...
                        if (numero1.Length > 1)
                        {
                            if (numero1.Substring(1).Contains('-'))
                            {
                                numero1 = numero1.Substring(0, numero1.IndexOf("-")+1);
                                break;
                            }
                        }
                    ...
                    if (double.TryParse(numero1, out cero))
                    {
                        ...
                        ecuacion = ecuacion.Replace($"{numero1}-{numero2}", resultado.ToString());
                        Console.WriteLine($"Ecuación if: {ecuacion}");
                    }
                    else
                    {
                        numero1 = "0";
                        ...
                        Console.WriteLine($"Ecuación else: {ecuacion}");
                    }
